...
`set-planning-profile` replaces all seven `unknown` values with `true` / `false` and derives `planning_tier`: all false is `lightweight`, any true is `complex`, and any unresolved answer remains `pending`. `meta.ui` is independent and must be the exact string `"true"` to enable the prototype gate; that opt-in also adds `prototype_manifest`. Historical tasks without `planning_contract_version: "2"` keep their existing start behavior.
