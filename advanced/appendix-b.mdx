...
| `set-planning-profile` | Save all seven boolean planning answers and derive `lightweight` / `complex`                      | `task.py set-planning-profile "$DIR" <seven flags>`                |
| `planning-status`      | Report profile, PRD, complex artifacts, interaction, database, and UI checks                      | `task.py planning-status "$DIR"`                                   |
| `prototype-status`     | Show the current UI prototype entry, digest, and approval state                                   | `task.py prototype-status "$DIR"`                                  |
| `approve-prototype`    | Bind explicit user approval to the current UI prototype digest                                    | `task.py approve-prototype "$DIR" "<approval-evidence>"`           |
| `start`                | Set as current task; validates planning contract and opted-in UI prototype before any side effect | `task.py start "$DIR"`                                             |
