...
| `planning`    | `task.py create`        | Planning artifacts are active; complex tasks need `design.md` and `implement.md`, while `meta.ui=true` tasks also need a complete, approved `prototype/`. |
...
├── check.jsonl
├── research/
└── prototype/          # UI tasks: manifest, entry, preview, assets
```
...
| `prototype/`      | UI prototype contract and digest-bound approval; created only when `meta.ui=true`.                       |
...
| `after_start`   | `task.py start` finishes                    | The task entered `in_progress`; a failed UI prototype gate does not emit this event.   |
