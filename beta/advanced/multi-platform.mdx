...
- **DeepSeek Harness** (`--dsh`): writes `.dsh/skills/` (user-invocable entry skills) plus the shared `.agents/skills/` layer; the agent loads skills by name through its skill-loader tool. No hook integration and no project-level sub-agent surface, so implement / check run inline in the main session.
