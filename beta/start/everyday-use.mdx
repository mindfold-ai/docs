...
trellis upgrade --tag beta   # switch from stable to the latest beta
...
<Note>
  ...
  not exist yet — join this beta directly with `npm install -g @mindfoldhq/trellis@beta`, then
  `trellis upgrade` is available for every future bump.
</Note>
