...
![Trellis 长期使用开发流](/images/trellis-mature-usage-flow-zh.png)
...
| Artifact          | 什么时候需要           | 作用                                                             |
| ----------------- | ---------------------- | ---------------------------------------------------------------- |
| `prd.md`          | 每个任务               | 固定顺序的目标、需求和用户可见结果；用户可见结果使用 checklist。 |
...
| `prototype/`      | 用户界面任务           | Manifest、可运行入口、静态预览、可选资源和绑定摘要的批准。       |
...
用户界面任务使用 `--meta ui=true` 创建。Trellis 会建立 `prototype/manifest.json`；planning 阶段完成其中声明的入口和预览，读取当前摘要和状态，向用户展示该版本，再用以下命令记录批准：
...
这个命令把 `task.json.status` 从 `planning` 改成 `in_progress`。对于 `meta.ui=true`，命令会先校验原型目录、目录内路径、当前摘要和明确批准；失败不会改变状态、活动指针，也不会触发 `after_start` hook。
