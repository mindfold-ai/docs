...
2. `prd.md` 跟你确认完，你输入 `continue` → 它记录七项规划画像；runtime 根据固定规则推导 `lightweight`、`complex` 或 `pending`，不再依赖 AI 自由判断。
...
```
create → plan artifacts → 启用时完成用户界面原型批准 → optional jsonl context → start → implement/check → finish → archive

create：                任务目录 + task.json + 默认 prd.md
plan artifacts：        每个任务都有 prd.md；复杂任务 start 前补 design.md + implement.md
用户界面原型批准：      meta.ui=true 的任务完成 prototype/，并把用户批准绑定到当前摘要
...
每个标准 Task 都以规划合同 v2 和 `planning_tier=pending` 开始。使用 `set-planning-profile` 回答交互、数据模型、公共合同、跨层、生命周期、安全/兼容/发布和技术取舍七项问题：全部为 `false` 推导为 `lightweight`；任一为 `true` 推导为 `complex`。用户界面工作增加 `--meta ui=true`，系统会创建 `prototype/manifest.json`，并在 PRD“用户可见结果”中生成入口、预览、状态和摘要管理块。
...
# 检查画像、PRD、复杂产物、交互图、数据库和 UI 合同
./.trellis/scripts/task.py planning-status "$TASK_DIR"

# 读取入口、重新计算的摘要和批准状态
./.trellis/scripts/task.py prototype-status "$TASK_DIR"

# 用户查看最新原型后，把批准绑定到当前摘要
./.trellis/scripts/task.py approve-prototype "$TASK_DIR" "在原型审查中批准"
...
- `meta.ui` / `meta.prototype_manifest` → `create --meta ui=true` 启用用户界面原型门禁，并写入标准 manifest 路径
- `meta.planning_contract_version` / `meta.planning_tier` 与七项画像字段 → `set-planning-profile` 推导 `pending`、`lightweight` 或 `complex`
...
`planning` / `in_progress` / `completed` 对应 `workflow.md` 的三个 Phase。`task.py start` 在把 `planning` 改写为 `in_progress` 前，会校验规划合同 v2；画像未决、PRD 编号/映射错误、复杂产物缺失、交互图不完整或数据库设计不完整时均无副作用拒绝。对于 `meta.ui=true`，它还会拒绝缺失、待批准、批准过期、路径越界或 PRD 引用不一致的原型。其他状态不动（`review` 这类自定义状态重启不会被清零）。
