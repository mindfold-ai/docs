...
2. 功能包含用户界面时，**AI** 使用 `--meta ui=true` 创建任务，完成 `prototype/` 内的入口和预览，向**用户**展示最新原型，并在 start 前用 `task.py approve-prototype` 记录批准。**AI** 同时配置 `implement.jsonl` / `check.jsonl`，把任务相关的 spec 加进来（Phase 1.3）。
