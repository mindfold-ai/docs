...
3. 可选：使用 Trellis 内置安装的 `trellis-spec-bootstrap`，从真实代码生成第一版 spec；其它对当前技术栈有用的 skills 可以一起接入。
...
3. 可选：使用 Trellis 内置安装的 `trellis-spec-bootstrap`，让 AI 从真实代码生成第一版 spec，再人工 review。
...
1. **用户** 用自然语言描述要做的功能；后续在 planning 中回答澄清问题、确认 scope → **AI** 先分类请求并询问是否创建 Trellis task。用户同意后，AI 加载 `trellis-brainstorm`，先检查相关代码/文档，再通过 `task.py create` 建任务，并把澄清出来的目标、用户需要的行为和可核验结果写到 `prd.md`；范围与证据分别放入配套产物（Phase 1.1）。
...
1. **用户** 用自然语言描述 bug 现象、已知复现路径、期望行为；后续在 planning 中跟 AI 一起 narrow down 根因假设 → **AI** 先分类请求并询问是否创建 Trellis task。用户同意后，AI 加载 `trellis-brainstorm`，先检查复现相关上下文，再通过 `task.py create` 建 bug fix 任务，并把目标、用户可理解的修复行为和可核验结果写到 `prd.md`；复现步骤、根因假设和回归测试要求放入配套产物（Phase 1.1）。
...
1. **用户** 用自然语言说明哪些 PR 反馈值得沉淀（粘贴 PR 链接或评论列表），后续在 planning 中跟 AI 一起按工程规则归类反馈 → **AI** 先分类请求并询问是否创建 Trellis task。用户同意后，AI 加载 `trellis-brainstorm`，先检查已提供的反馈，再通过 `task.py create` 建 spec 完善任务，并把目标 review pattern、用户需要的行为和可核验结果写到 `prd.md`；范围与证据分别放入配套产物（Phase 1.1）。
...
| 阶段 | 目标             | 退出标准                                                                                                                                                                                                                                   |
| ---- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | 试点一个仓库     | 用 specs、checks、journal 完成一个真实任务。spec 起点两个选项：从 [spec template marketplace](/zh/start/install-and-first-task#官方-marketplace) 拉一个匹配技术栈的 spec 包；或用 `trellis-spec-bootstrap` 让 AI 从真实代码生成第一版 spec |
...
将用户目标、所需行为和可见结果留在 PRD；将实现细节和仓库证据移入专属产物。
