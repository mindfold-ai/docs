...
| `set-planning-profile` | 一次保存七项布尔规划画像并推导 `lightweight` / `complex`                   | `task.py set-planning-profile "$DIR" <七项参数>`                   |
| `planning-status`      | 报告画像、PRD、复杂产物、交互、数据库和 UI 检查                            | `task.py planning-status "$DIR"`                                   |
| `prototype-status`     | 显示当前用户界面原型入口、摘要和批准状态                                   | `task.py prototype-status "$DIR"`                                  |
| `approve-prototype`    | 把明确的用户批准绑定到当前用户界面原型摘要                                 | `task.py approve-prototype "$DIR" "<approval-evidence>"`           |
| `start`                | 置为当前任务；任何副作用前先校验规划合同和已启用的 UI prototype            | `task.py start "$DIR"`                                             |
