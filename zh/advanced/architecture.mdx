...
| `planning`    | `task.py create`  | Planning artifact 阶段；复杂任务需要 `design.md` 和 `implement.md`，`meta.ui=true` 的任务还需要完整且已批准的 `prototype/`。 |
...
├── check.jsonl
├── research/
└── prototype/          # 用户界面任务：manifest、入口、预览、资源
```
...
| `prd.md`          | 固定顺序的目标、需求和用户可见结果；用户可见结果使用 checklist。    |
...
| `prototype/`      | 用户界面原型合同和绑定摘要的批准；仅 `meta.ui=true` 时创建。        |
...
| `after_start`   | `task.py start` 结束                  | 任务进入 `in_progress`；用户界面原型门禁失败时不触发。      |
