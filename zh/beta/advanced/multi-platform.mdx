...
- **DeepSeek Harness**（`--dsh`）：把共享工作流写到 `.agents/skills/`，把用户可调用的入口 skill 写到 `.dsh/skills/trellis-*`，并把无名称冲突、不可由用户调用的子代理角色写到 `.dsh/skills/trellis-agent-*`。主会话派发 DSH 原生、可续接的 `subagent`，每个子代理只加载一个角色 skill，并主动拉取任务上下文。若可选的 [`dsh-trellis`](https://github.com/SajoLuo/dsh-trellis) 伴侣插件提供 `trellis_wait`，依赖阶段会等待原生 settlement 事件，期间可继续独立工作；未安装插件时，子代理从一开始就在前台派发。两条路径都不使用 shell sleep 或轮询，`trellis init --dsh` 也不会安装 profile 插件。
  ...
  DSH 还会继承启动进程的普通环境变量。从已有活跃 Trellis 会话中启动 DSH 时，外层的 `TRELLIS_CONTEXT_ID` 原本会覆盖 DSH 的原生会话 ID。DSH 会为每个受管 shell 重建完整的 `DSH_*` 命名空间，因此即使未安装插件，beta 适配也会把同时存在的 `DSH_SHELL=1` 与 `DSH_SESSION_ID` 视为当前 DSH 身份，并优先于继承的通用 override 解析。可选伴侣插件还会在需要转发不同于 shell 自身 session ID 的子代理身份时，为每次执行发布可信的 `DSH_TRELLIS_CONTEXT_ID`。

  每次 `dsh --profile headless` 调用都会创建新的 DSH 会话。请在同一会话中完成任务流程，或显式续接返回的 session id；多个独立 headless 调用不会自动共享 active-task 指针。
