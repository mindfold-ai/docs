...
- **DeepSeek Harness**（`--dsh`）：写入 `.dsh/skills/`（用户可调用的入口 skill）以及共享的 `.agents/skills/` 层；agent 通过 skill-loader 工具按名字加载 skill。无 hook 集成，也没有项目级 sub-agent 定义面，因此 implement / check 阶段在主会话内联执行。
