...
![Trellis mature usage flow](/images/trellis-mature-usage-flow-en.png)
...
├── check.jsonl
├── research/
└── prototype/          # created for --meta ui=true tasks
```
...
| `prototype/`      | UI tasks                    | Manifest, runnable entry, static preview, optional assets, and digest-bound approval.   |
...
UI tasks are created with `--meta ui=true`. Trellis creates `prototype/manifest.json`; planning completes the declared entry and preview, reads their current digest and status, shows that version to the user, and records approval with:

```bash
python3 ./.trellis/scripts/task.py prototype-status <task-dir>
python3 ./.trellis/scripts/task.py approve-prototype <task-dir> "<approval-evidence>"
```
...
This changes `task.json.status` from `planning` to `in_progress`. For `meta.ui=true`, it first validates the prototype directory, contained paths, current digest, and explicit approval. A failure leaves status, the active pointer, and `after_start` hooks unchanged.
