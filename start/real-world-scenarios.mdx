...
3. Optional: use the bundled `trellis-spec-bootstrap` skill to draft first-pass specs from the real codebase, plus any other skills that match the stack. Trellis installs this bundled skill automatically.
...
3. Optional: use the bundled `trellis-spec-bootstrap` skill to draft first-pass specs from the real codebase, then review the output by hand. Trellis installs this bundled skill automatically.
...
2. If the feature includes UI work, **AI** creates it with `--meta ui=true`, completes the entry and preview under `prototype/`, shows the latest prototype to the **User**, and records approval with `task.py approve-prototype` before start. **AI** also configures `implement.jsonl` / `check.jsonl` with the specs the task touches (Phase 1.3).
