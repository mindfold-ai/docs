...
2. Once `prd.md` looks right, type `continue` → it records all seven planning-profile answers; the runtime derives `lightweight`, `complex`, or `pending` instead of relying on a free-form AI decision.
...
```
create → plan artifacts → UI prototype approval when enabled → optional jsonl context → start → implement/check → finish → archive
...
UI prototype approval: meta.ui=true tasks complete prototype/ and bind user approval to its current digest
...
Every standard task starts with planning contract v2 and `planning_tier=pending`. Use `set-planning-profile` to answer interaction, data model, public contract, cross-layer, lifecycle, security/compatibility/rollout, and technical-tradeoff questions. All `false` derives `lightweight`; any `true` derives `complex`. For UI work, add `--meta ui=true`; this creates `prototype/manifest.json` and a PRD User-visible Outcomes block for entry, preview, status, and digest.
...
```bash
# Save all seven answers together and derive the tier
./.trellis/scripts/task.py set-planning-profile "$TASK_DIR" \
...
# Inspect the profile, PRD, complex artifacts, interaction, database, and UI checks
./.trellis/scripts/task.py planning-status "$TASK_DIR"

# Read the entry, freshly computed digest, and approval status
./.trellis/scripts/task.py prototype-status "$TASK_DIR"

# After the user reviews the latest UI prototype, bind approval to its digest
./.trellis/scripts/task.py approve-prototype "$TASK_DIR" "approved in prototype review"
...
- `meta.ui` / `meta.prototype_manifest` → `create --meta ui=true` enables the UI prototype gate and writes the standard manifest path
- `meta.planning_contract_version` / `meta.planning_tier` and seven profile fields → `set-planning-profile` derives `pending`, `lightweight`, or `complex`
...
`planning` / `in_progress` / `completed` align with the three phases in `workflow.md`. Before rewriting `planning` to `in_progress`, `task.py start` validates planning contract v2: unresolved profiles, invalid PRD numbering/mapping, missing complex artifacts, incomplete interaction diagrams, and incomplete database designs are rejected without side effects. For `meta.ui=true`, it also rejects missing, pending, stale, escaping, or PRD-inconsistent prototype artifacts. Non-planning statuses (`in_progress`, `review`, `completed`) are left untouched, so re-starting a task in `review` doesn't clobber its state.
