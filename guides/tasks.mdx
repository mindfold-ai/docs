...
Write what users need and what they can verify. Use the fixed Goal, Requirements, and User-visible Outcomes sections; keep technical design, execution, and evidence in their companion artifacts.
...
New task PRDs use `Goal`, `Requirements`, and `User-visible Outcomes` in that order. Goals are ordered lists and outcomes are verifiable checklists. Keep technical design in `design.md`, execution in `implement.md`, and source evidence in `research/`. UI tasks require explicit prototype approval before implementation starts.
